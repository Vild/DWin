...
private enum string[] keywords = [
		"function", "continue", "__FILE__", "__LINE__", // these two are special to the lexer
		"foreach", "json!q{", "default", "finally", "return", "static",
		"struct", "import", "module", "assert", "switch", "while", "catch", "throw", "scope", "break", "super",
		"class", "false", "mixin", "super", "macro", "auto", // provided as an alias for var right now, may change later
		"null", "else", "true", "eval", "goto", "enum", "case",
		"cast", "var", "for", "try", "new", "if", "do",
	];
		...
		mainLoop: while (text.length) {
			...
			if (text[0] == ' ' || text[0] == '\t' || text[0] == '\n') {
				...
				if (!found) {
					...
					while (pos < text.length && ((text[pos] >= 'a' && text[pos] <= 'z') || (text[pos] == '_')
							|| (text[pos] >= 'A' && text[pos] <= 'Z') || (text[pos] >= '0' && text[pos] <= '9'))) {
						pos++;
					}
...
class MacroPrototype : PrototypeObject {
	...
	this(var func) {
		this.func = func;
		this._properties["opCall"] = (var _this, var[] args) { return func.apply(_this, args); };
	}
}
		...
		foreach (pos, char c; s) {
			...
			if (inEscape) {
				...
				case '\\':
					...
					break;
				default:
					throw new ScriptCompileException("literal escape unknown " ~ c, 0, null, 0);
				}
			}
		}
		...
		v._function = (var _this, var[] args) {
			...
			if (arguments)
				foreach (i, identifier; arguments.identifiers) {
					...
					if (i < args.length && !(args[i].payloadType() == var.Type.Object
							&& args[i]._payload._object is DefaultArgumentDummyObject))
						argumentsScope._getMember(identifier, false, true) = args[i];
					else if (arguments.initializers[i]!is null)
...
class CastExpression : Expression {
	...
	override InterpretResult interpret(PrototypeObject sc) {
		...
		foreach (possibleType; CtList!("int", "long", "float", "double", "real", "char", "dchar", "string", "int[]",
				"string[]", "float[]")) {
	...
	if (!tokens.empty) {
		...
		funcLoop: while (!tokens.empty) {
			...
			if (peek.type == ScriptToken.Type.symbol) {
				...
				return e; // again, we don't know, so just punt it down the line
		}
		return e;
	}
...
Expression parseFactor(MyTokenStreamHere)(ref MyTokenStreamHere tokens) {
	...
	loop: while (!tokens.empty) {
		...
		if (peek.type == ScriptToken.Type.symbol) {
			...
			throw new Exception("Got " ~ peek.str ~ " when expecting symbol");
	}
...
CompoundStatementRange!MyTokenStreamHere //Expression[]
...
void repl(var globals) {
	...
	auto variables = (globals.payloadType() == var.Type.Object && globals._payload._object !is null) ? globals._payload._object
		: new PrototypeObject();

	// we chain to ensure the priming popFront succeeds so we don't throw here
	auto tokens = lexScript(chain(["var __skipme = 0;"], map!((a) => a.idup)(stdin.byLine)), "stdin");
