...
alias Key = xcb_keysym_t;

private static immutable Key[string] Keys;

private xcb_keysym_t Alt_L, Alt_R, Super_L, Super_R, Hyper_L, Hyper_R, Meta_L, Meta_R, Mode_switch, Num_Lock, Scroll_Lock;

void InitKeys() {
	Alt_L = ParseKey("Alt_L");
	Alt_R = ParseKey("Alt_R");
	Super_L = ParseKey("Super_L");
	Super_R = ParseKey("Super_R");
	Hyper_L = ParseKey("Hyper_L");
	Hyper_R = ParseKey("Hyper_R");
	Mode_switch = ParseKey("Mode_switch");
	Num_Lock = ParseKey("Num_Lock");
	Scroll_Lock = ParseKey("Scroll_Lock");
}
...
Key ParseKey(string key) {
	if (auto val = key in Keys)
		...
		return 0xffffff; //Keys["VoidSymbol"];
}
...
Modifier KeyCodeToModifier(XCB xcb, xcb_keycode_t keycode) {
	...
	if (reply && reply.keycodes_per_modifier > 0) {
		if (xcb_keycode_t* modifierArray = xcb_get_modifier_mapping_keycodes(reply)) {
			for (uint i = 0; i < xcb_get_modifier_mapping_keycodes_length(reply) / reply.keycodes_per_modifier; i++) {
				for (uint j = 0; j < reply.keycodes_per_modifier; j++) {
					immutable xcb_keycode_t modifier = modifierArray[i * reply.keycodes_per_modifier + j];
...
Modifier KeySymToModifier(XCB xcb, xcb_keysym_t keySym) {
	...
	for (ulong keyCode = setup.min_keycode; keyCode <= setup.max_keycode; keyCode++)
		for (int col = 0; col < KEYSYMS_PER_KEYCODE; col++)
			if (xcb_key_symbols_get_keysym(xcb.Symbols, cast(ubyte)keyCode, col) == keySym)
				modfield |= KeyCodeToModifier(xcb, cast(ubyte)keyCode);

	return modfield;
}

Modifier ParseModifier(XCB xcb, string modifier) {
	if (modifier == "shift")
		return XCB_MOD_MASK_SHIFT;
	else if (modifier == "control" || modifier == "ctrl")
		return XCB_MOD_MASK_CONTROL;
	else if (modifier == "alt")
		return (KeySymToModifier(xcb, Alt_L) | KeySymToModifier(xcb, Alt_R));
	else if (modifier == "super")
		return (KeySymToModifier(xcb, Super_L) | KeySymToModifier(xcb, Super_R));
	else if (modifier == "hyper")
		return (KeySymToModifier(xcb, Hyper_L) | KeySymToModifier(xcb, Hyper_R));
	else if (modifier == "meta")
		return (KeySymToModifier(xcb, Meta_L) | KeySymToModifier(xcb, Meta_R));
	else if (modifier == "mode_switch")
		return KeySymToModifier(xcb, Mode_switch);
